...
async function patchAiCluster($formFilters, $formValues, pk, success, error) {
  ...
  patchAiClusterVals(pk == null ? $.deparam(window.location.search ? window.location.search.substring(1) : window.location.search) : [{name:'fq', value:'pk:' + pk}], vals, target, success, error);
}
...
async function putimportAiCluster($formValues, pk, success, error) {
...
async function websocketAiCluster(success) {
  window.eventBus.onopen = function () {
    ...
    window.eventBus.registerHandler('websocketAiCluster', function (error, message) {
      ...
      var id = json['id'];
      var pk = json['pk'];
      var pkPage = document.querySelector('#AiClusterForm :input[name=pk]')?.value;
      var pks = json['pks'];
      ...
      var $i = document.createElement('<i class="fa-regular fa-server"></i>');
      var $headerSpan = document.createElement('<span>').setAttribute('class', '').text('modify AI clusters in ' + json.timeRemaining);
  ...
  if(pk != null && vars.length > 0) {
    ...
    $.get(uri, {}, function(data) {
      var $response = $("<html/>").html(data);
        ...
        jsWebsocketAiCluster(pk, vars, $response);

        window.aiCluster = JSON.parse($response.querySelector('.pageForm .aiCluster')?.value);
...
function pageGraphAiCluster(apiRequest) {
  ...
  if(r) {
    ...
    if(window.mapAiCluster) {
      ...
      window.listAiCluster.forEach((aiCluster, index) => {
        if(aiCluster.location) {
          var shapes = [];
          if(Array.isArray(aiCluster.location))
            shapes = shapes.concat(aiCluster.location);
          else
            shapes.push(aiCluster.location);
          shapes.forEach(function(shape, index) {
            var features = [{
              "type": "Feature"
              , "properties": aiCluster
              , "geometry": shape
              , "index": index
            }];
...
function animateStats() {
  document.querySelector('#pageSearchVal-fqAiCluster_time').text('');
