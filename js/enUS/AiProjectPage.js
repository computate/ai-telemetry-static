...
async function patchAiProject($formFilters, $formValues, pk, success, error) {
  ...
  patchAiProjectVals(pk == null ? $.deparam(window.location.search ? window.location.search.substring(1) : window.location.search) : [{name:'fq', value:'pk:' + pk}], vals, target, success, error);
}
...
async function putimportAiProject($formValues, pk, success, error) {
...
async function websocketAiProject(success) {
  window.eventBus.onopen = function () {
    ...
    window.eventBus.registerHandler('websocketAiProject', function (error, message) {
      ...
      var id = json['id'];
      var pk = json['pk'];
      var pkPage = document.querySelector('#AiProjectForm :input[name=pk]')?.value;
      var pks = json['pks'];
      ...
      var $i = document.createElement('<i class="fa-regular fa-school"></i>');
      var $headerSpan = document.createElement('<span>').setAttribute('class', '').text('modify AI projects in ' + json.timeRemaining);
  ...
  if(pk != null && vars.length > 0) {
    ...
    $.get(uri, {}, function(data) {
      var $response = $("<html/>").html(data);
        ...
        jsWebsocketAiProject(pk, vars, $response);

        window.aiProject = JSON.parse($response.querySelector('.pageForm .aiProject')?.value);
...
function pageGraphAiProject(apiRequest) {
  ...
  if(r) {
    ...
    if(window.mapAiProject) {
      ...
      window.listAiProject.forEach((aiProject, index) => {
        if(aiProject.location) {
          var shapes = [];
          if(Array.isArray(aiProject.location))
            shapes = shapes.concat(aiProject.location);
          else
            shapes.push(aiProject.location);
          shapes.forEach(function(shape, index) {
            var features = [{
              "type": "Feature"
              , "properties": aiProject
              , "geometry": shape
              , "index": index
            }];
...
function animateStats() {
  document.querySelector('#pageSearchVal-fqAiProject_time').text('');
