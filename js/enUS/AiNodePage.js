...
async function patchAiNode($formFilters, $formValues, pk, success, error) {
  ...
  patchAiNodeVals(pk == null ? $.deparam(window.location.search ? window.location.search.substring(1) : window.location.search) : [{name:'fq', value:'pk:' + pk}], vals, target, success, error);
}
...
async function putimportAiNode($formValues, pk, success, error) {
...
async function websocketAiNode(success) {
  window.eventBus.onopen = function () {
    ...
    window.eventBus.registerHandler('websocketAiNode', function (error, message) {
      ...
      var id = json['id'];
      var pk = json['pk'];
      var pkPage = document.querySelector('#AiNodeForm :input[name=pk]')?.value;
      var pks = json['pks'];
      ...
      var $i = document.createElement('<i class="fa-regular fa-computer"></i>');
      var $headerSpan = document.createElement('<span>').setAttribute('class', '').text('modify AI nodes in ' + json.timeRemaining);
  ...
  if(pk != null && vars.length > 0) {
    ...
    $.get(uri, {}, function(data) {
      var $response = $("<html/>").html(data);
        ...
        jsWebsocketAiNode(pk, vars, $response);

        window.aiNode = JSON.parse($response.querySelector('.pageForm .aiNode')?.value);
...
function pageGraphAiNode(apiRequest) {
  ...
  if(r) {
    ...
    if(window.mapAiNode) {
      ...
      window.listAiNode.forEach((aiNode, index) => {
        if(aiNode.location) {
          var shapes = [];
          if(Array.isArray(aiNode.location))
            shapes = shapes.concat(aiNode.location);
          else
            shapes.push(aiNode.location);
...
function animateStats() {
  document.querySelector('#pageSearchVal-fqAiNode_time').text('');
