...
async function patchHub($formFilters, $formValues, target, tenantId, success, error) {
  ...
  patchHubVals(tenantId == null ? deparam(window.location.search ? window.location.search.substring(1) : window.location.search) : [{name:'fq', value:'tenantId:' + tenantId}], vals, target, success, error);
}
...
async function deleteHub(target, tenantId, success, error) {
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
      addGlow(target, jqXhr);
      var url = data['editPage'];
  ...
  fetch(
    '/en-us/api/hub/' + encodeURIComponent(tenantId)
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
      , method: 'DELETE'
    }).then(response => {
...
async function putimportHub($formValues, target, tenantId, success, error) {
  ...
  if(tenantId != null && vars.length > 0) {
    ...
    fetch(uri).then(response => {
      response.text().then(text => {
        ...
        jsWebsocketHub(tenantId, vars, $response);
